...
public class ProcessTreeVisualisation {
	...
	private IMLog log;
	private HashMap <String, Object[][]> activity_attributes;

	public Triple<Dot, ProcessTreeVisualisationInfo, TraceViewEventColourMap> fancy(IvMEfficientTree tree,
			AlignedLogVisualisationData data, ProcessTreeVisualisationParameters parameters, AttributesInfo attributesInfo, Boolean includeAttributes, IMLog log, HashMap <String, Object[][]> activity_attributes)
			throws UnknownTreeNodeException {
		return fancy(new IvMModel(tree), data, parameters, attributesInfo, includeAttributes, log, activity_attributes);
	}
	...
	public Triple<Dot, ProcessTreeVisualisationInfo, TraceViewEventColourMap> fancy(IvMModel model,
		...
		this.log = log;
		this.activity_attributes = activity_attributes;
	...
	private LocalDotNode convertActivity(EfficientTree tree, int unode, Triple<String, Long, Long> cardinality) {
		...
		attributesToDisplay = activity_attributes;
					...
					for(int i = 0; i < attributesOfActivity.length;i++) {
							...
							for(int j = 1; j < attributesOfActivity[i].length; j++) {
								...
								if(attributesOfActivity[i][j] != null) {
									...
									if(attributesOfActivity[i][j].equals("Mean")) {
									...
									else if(attributesOfActivity[i][j].equals("Minimum")) {
									...
									else if (attributesOfActivity[i][j].equals("Maximum")) {
									...
									else if (attributesOfActivity[i][j].equals("Median")) {
										DoubleStream sortedValues = ((List<Double>) currAttributeValues).stream().mapToDouble(Double::doubleValue).sorted();
										double median = ((List<Double>) currAttributeValues).size()%2 == 0?
											sortedValues.skip(((List<Double>) currAttributeValues).size()/2-1).limit(2).average().getAsDouble():        
											sortedValues.skip(((List<Double>) currAttributeValues).size()/2).findFirst().getAsDouble();
											label_content += "<tr><td>" + "Median " + currAttribute + ": " + df.format(median) + " " + unit + "</td></tr>";
									}
									else {
										...
										for (Object e : currAttributeValues) {
											if(e != null) {
												if (e.toString().equals(attributesOfActivity[i][j].toString())) {
													countCategory++;
												}
											}
										}
										//System.out.println(countCategory);
											...
											label_content +=  "<tr><td>" + currAttribute + " " + attributesOfActivity[i][j].toString() + " " + ": "  + df.format(ratioCategory) + "%" + "</td></tr>";
